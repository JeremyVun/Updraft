...
interface Butterfly {
  ...
  tumble: number;
  carried: number;
  /** Folded among the flowers until life reaches the patch. */
  asleep: boolean;
  wakeDelay: number;
}
...
export class Butterflies {
  ...
  update(dt: number, time: number, s: Stimuli): void {
    ...
    for (const b of this.list) {
      if (dormant(s, b.x, b.z)) continue;
      if (b.asleep) {
        if (s.life(b.patch.x, b.patch.z) < 0.6 || (b.wakeDelay -= dt) > 0) continue;
        b.asleep = false;
        b.y = this.top(b.x, b.z) + 0.1;
        b.vy = 2.2;
        b.state = 'flutter';
        b.timer = range(b.rand, 1, 3);
      }
      const rand = b.rand;
      ...
      this.instances.set(0, drawn, b.x, b.y, b.z, b.yaw);
      this.instances.set(1, drawn, b.flap, b.pitch, b.roll, 0);
      this.instances.set(2, drawn, b.kind, b.size, b.seed, nudge);
      drawn++;
    }
