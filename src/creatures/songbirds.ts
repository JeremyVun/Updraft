...
interface Flock {
  ...
  centreX: number;
  centreZ: number;
  /** Away somewhere until life returns to the flock's range; then they fly in. */
  asleep: boolean;
  wakeDelay: number;
  dormant: boolean;
}
...
export class Songbirds {
  ...
  private life: (x: number, z: number) => number = () => 1;
  /** The tree has leaves to sit among. */
  ...
  update(dt: number, time: number, s: Stimuli): void {
    ...
    this.life = s.life;
    this.treeReady = s.life(this.habitat.treeBase.x, this.habitat.treeBase.z) > 0.8;
    for (const flock of this.flocks) {
      if (flock.asleep) {
        if (s.life(flock.home.x, flock.home.z) < 0.7 || (flock.wakeDelay -= dt) > 0) continue;
        this.arrive(flock);
      }
      flock.dormant = dormant(s, flock.centreX, flock.centreZ);
      if (flock.dormant) continue;
  ...
  private arrive(flock: Flock): void {
    ...
    const a = Math.PI + (rand() - 0.5) * 2.2;
    const d = range(rand, 60, 80);
    const x = flock.home.x + Math.sin(a) * d;
    const z = flock.home.z + Math.cos(a) * d;
    const y = Math.max(this.habitat.ground(x, z), 0) + 20;
    for (const b of flock.birds) {
      b.x = x + (rand() - 0.5) * 4;
      b.y = y + (rand() - 0.5) * 2;
      b.z = z + (rand() - 0.5) * 4;
    }
    this.takeoff(flock, null);
    for (const b of flock.birds) b.delay = 0;
  }
  ...
  private threat(flock: Flock, s: Stimuli): { x: number; z: number } | null {
    ...
    for (const b of flock.birds) {
      ...
      if (s.walker && Math.hypot(s.walker.x - b.x, s.walker.z - b.z) < (b.mode === 'perch' ? 2.5 : 4.5)) return { x: s.walker.x, z: s.walker.z };
    }
    return null;
  }
  ...
  private takeoff(flock: Flock, threat: { x: number; z: number } | null): void {
    ...
    const toTree = this.treeReady && this.habitat.perches.length >= birds.length && (flock.mode === 'ground' ? rand() < (threat ? 0.6 : 0.5) : rand() < 0.2);
    const ends: { x: number; y: number; z: number; perch: Perch | null }[] = [];
    for (const b of birds) if (b.perch) this.taken.delete(b.perch);
    if (toTree) {
      const first = this.freePerch(rand, threat, flock);
      if (first) {
  ...
  /** A free perch within reach of the flock's range, away from the disturbance, preferring the side the player sees. */
  private freePerch(rand: Rng, threat: { x: number; z: number } | null, flock: Flock): Perch | null {
    const reach = flock.home.radius + 40;
    const free = this.habitat.perches.filter(
      (p) =>
        !this.taken.has(p) &&
        Math.hypot(p.x - flock.home.x, p.z - flock.home.z) < reach &&
        (!threat || Math.hypot(p.x - threat.x, p.z - threat.z) > 8),
    );
    const eye = this.eye;
  ...
  private groundSpot(flock: Flock, threat: { x: number; z: number } | null): [number, number] {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (!this.habitat.forage(px, pz) || this.life(px, pz) < 0.6) continue;
