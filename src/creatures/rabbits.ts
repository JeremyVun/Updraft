...
interface Rabbit {
  ...
  nextBlink: number;
  groomEar: number;
  /** Still underground, waiting for life to reach the warren. */
  asleep: boolean;
  wakeDelay: number;
}
...
export class Rabbits {
  ...
  private findSpot(rand: Rng, x: number, z: number, min: number, max: number, tries: number): [number, number] | null {
    ...
    for (let i = 0; i < tries; i++) {
      ...
      if (!this.habitat.meadow(px, pz) || this.crowded(px, pz, null) || this.life(px, pz) < 0.6) continue;
  ...
  update(dt: number, time: number, s: Stimuli): void {
    ...
    for (const r of this.list) {
      if (dormant(s, r.x, r.z)) continue;
      if (r.asleep) {
        if (s.life(r.x, r.z) < 0.75 || (r.wakeDelay -= dt) > 0) continue;
        this.emerge(r);
      }
      ...
      if (s.walker) threat += Math.max(0, 1 - Math.hypot(s.walker.x - r.x, s.walker.z - r.z) / 8) * 1.6;
  ...
  private emerge(r: Rabbit): void {
    r.asleep = false;
    r.squash.value = 0.3;
    r.squash.velocity = 3;
    r.earL.value = r.earR.value = 1.4;
    r.activity = 'look';
    r.timer = range(r.rand, 1.4, 2.6);
    r.gazeUp = false;
  ...
  private react(r: Rabbit, threat: number, speed: number, wx: number, wz: number, curious: boolean, s: Stimuli): void {
    if (threat > 0.85 && !r.fleeing) {
      ...
      const walkerNear = s.walker && Math.hypot(s.walker.x - r.x, s.walker.z - r.z) < 8;
      const from = s.gustAt ?? (walkerNear ? s.walker : null);
