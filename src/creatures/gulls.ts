...
interface Gull {
  ...
  nextLook: number;
  headGoal: number;
  /** Away over the sea until the breeze returns. */
  asleep: boolean;
  wakeDelay: number;
}
...
export class Gulls {
  ...
  update(dt: number, time: number, s: Stimuli): void {
    ...
    for (const g of this.list) {
      if (g.asleep) {
        if (s.breeze < 0.5 || (g.wakeDelay -= dt) > 0) continue;
        this.arrive(g);
      }
      if (dormant(s, g.x, g.z)) continue;
      const rand = g.rand;
      ...
      this.instances.set(0, shown, g.x, g.y, g.z, g.yaw);
      this.instances.set(1, shown, g.pitch, g.bank, 0, 0);
      this.instances.set(2, shown, inner, outer, g.sweep, g.headYaw);
      shown++;
    }
  ...
  private arrive(g: Gull): void {
    g.asleep = false;
    const a = g.rand() * Math.PI * 2;
    const d = range(g.rand, 140, 190);
    g.x = g.homeX + Math.cos(a) * d;
    g.z = g.homeZ + Math.sin(a) * d;
    g.y = range(g.rand, 26, 36);
    g.yaw = Math.atan2(g.homeX - g.x, g.homeZ - g.z);
  }
