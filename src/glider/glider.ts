...
void main() {
  ...
  float through = max(-ndl, 0.0) * 0.7;
  float rim = pow(1.0 - max(dot(N, V), 0.0), 3.0);
  vec3 col = alb * (hemiLight(N) * 1.1 + uSunColor * (max(ndl, 0.0) + through)) + uSunColor * rim * 0.3;
...
const HOME = new THREE.Vector3(-8, 0, 8);
const ISLAND_CENTRE = new THREE.Vector2(-6, -14);
/** Beyond this distance from the island the glider banks around and comes home. */
...
export class Glider {
  ...
  constructor(
    ...
    const sg = new THREE.PlaneGeometry(4, 4);
  ...
  reset(): void {
    this.position.set(HOME.x, Math.max(heightAt(HOME.x, HOME.z), 0) + 9, HOME.z);
  ...
  update(dt: number, time: number): void {
    ...
    const clearance = ground > GRASS_LINE ? 2.2 : 0.7;
    const floor = surface + clearance;
    const altitude = p.y - floor;
    const windSpeed = Math.hypot(w.x, w.z);

    const airspeed = Math.hypot(v.x, v.z);
    const liftForce = w.energy * 8 + w.lift * 10 + Math.max(0, windSpeed - 4) * 0.3 + Math.max(0, airspeed - 5) * 0.55;
    ...
    const vyTarget = resting ? 0 : liftForce - 1.2;
    v.y += (vyTarget - v.y) * (1 - Math.exp(-dt * 1.4));

    const r = Math.hypot(p.x - ISLAND_CENTRE.x, p.z - ISLAND_CENTRE.y);
    if (r > HOME_RADIUS) {
      const pull = Math.min((r - HOME_RADIUS) * 0.5, 14) * dt;
      v.x -= ((p.x - ISLAND_CENTRE.x) / r) * pull;
      v.z -= ((p.z - ISLAND_CENTRE.y) / r) * pull;
    }
    if (p.y > 42) v.y -= (p.y - 42) * dt * 1.5;
  ...
  brush(camera: THREE.Camera, a: THREE.Vector2, b: THREE.Vector2, gust: number, dir: THREE.Vector2, charge: number, dt: number): void {
    ...
    if (gust > 1) {
      const k = 1 - Math.exp(-dt * 9 * f);
      this.velocity.x += (dir.x * gust * 1.05 - this.velocity.x) * k;
      this.velocity.z += (dir.y * gust * 1.05 - this.velocity.z) * k;
      this.velocity.y += gust * 0.35 * f * dt * 9;
    }
    if (charge > 0) this.velocity.y += charge * 14 * f * dt;
  }

  private updateTrail(trail: Ribbon, side: number, speed: number): void {
    const tip = this.scratch.set(side * 1.7, 0.24, -1.4).applyMatrix4(this.group.matrixWorld);
