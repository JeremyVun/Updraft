...
float surfReach(float n, vec2 xz) {
  return mix(1.0, 3.4, hash12(vec2(n, 3.7))) * (0.7 + 0.6 * vnoise(xz * 0.045 + n * 1.37));
}
...
vec4 beachSwash(vec2 xz, float x, vec2 inland, Footprint fp) {
  ...
  float lip = cover * exp(-depth / 0.25) * mix(0.35, 1.0, rushing);
  float sheet = cover * exp(-p * 3.5) * 0.5 * smoothstep(0.2, 0.7, vnoise(xz * 0.35 + n * 5.3));
...
vec3 shadeSwash(vec3 col, vec4 swash, vec3 wpos, float sunVis) {
  ...
  float sheet = swash.x;
  col *= mix(vec3(1.0), exp(-vec3(0.5, 0.13, 0.1) * swash.w * 0.35), sheet);
  vec3 R = reflect(-V, N);
  R.y = abs(R.y);
