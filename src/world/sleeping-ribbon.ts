...
export class CurtainRibbon {
  ...
  constructor(readonly knot: THREE.Vector3, readonly end: THREE.Vector3) {
    ...
    this.mesh = new THREE.Mesh(geo, new THREE.ShaderMaterial({
      ...
      fragmentShader: `${ATMO_GLSL}\nin vec3 vWorld;
        ...
        vec3 col=vec3(0.78,0.25,0.12)*(vec3(0.24,0.16,0.10)+hemiLight(n)*1.5+uSunColor*max(0.2,dot(n,uSunDir))*0.5+lampLight(vWorld,n)+dawnLight(vWorld,n));
  ...
  reset(): void {
    this.pull = 0; this.released = false; this.held = false; this.elapsed = 0; this.freeAge = 0;
    this.update(0, 0, 0);
  }
  ...
  update(dt: number, time: number, open: number): void {
    ...
    if (this.released && !this.held) this.freeAge += dt;
    this.gripAt(this.pull, this.tip);
    const loose = THREE.MathUtils.smoothstep(this.elapsed, 0, 2.5);
    const free = THREE.MathUtils.smoothstep(this.freeAge, 0, 0.7);
    for (let s = 0; s < STRIPS; s++) for (let i = 0; i <= STEPS; i++) {
      ...
      if (s < 2) {
        ...
        const side = s === 0 ? -1 : 1;
        p.x += side * Math.sin(u * Math.PI) * 0.48 * bow + side * open * 0.65;
        p.y += Math.sin(u * Math.PI * 2) * 0.27 * bow;
      ...
      // A broad fabric end, flaring gently into a swallowtail rather than reading as a wire.
      const width = (s < 2 ? 0.105 : 0.13) * (1 + Math.max(0, u - 0.78) * 1.8);
      if (s >= 2 && u > 0.92) p.y += (u - 0.92) * 0.55;
