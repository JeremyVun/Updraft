...
// second run of this one are compared frame by frame: the wind the sail reads back differs from run to run, so only
// the position, which the contacts decide, can be compared across runs.
// BASE=<this checkout's Vite> [OLD=<previous build's Vite>] node tools/boat-mooring-check.mjs
...
const LABELS = ['time', 'x', 'y', 'z', 'qx', 'qy', 'qz', 'qw', 'pitch', 'roll', 'yaw'];
// Frames paired by game time; the largest difference in each pose component.
function compare(a, b) {
  const at = new Map(a.poses.map(pose => [pose[0], pose]));
  const pairs = b.poses.filter(pose => at.has(pose[0]));
  const max = Object.fromEntries(LABELS.slice(1).map(k => [k, 0]));
  for (const pose of pairs) LABELS.slice(1).forEach((k, i) => { max[k] = Math.max(max[k], Math.abs(pose[i + 1] - at.get(pose[0])[i + 1])); });
  return { frames: pairs.length, max };
}
try {
  for (const run of runs) {
    ...
    const entry = report[run.name] = { frames: now.poses.length, skipped: now.skipped, tested: now.tested, measured: now.measured,
      closestGap: now.closest, violations: now.violations.slice(0, 5), errors: now.errors, updateMs: mean(now.ms) };
    if (old) {
      const before = await record(browser, old, run, false);
      const again = await record(browser, base, run, false);
      entry.previousUpdateMs = mean(before.ms);
      entry.againUpdateMs = mean(again.ms);
      entry.againstPrevious = compare(before, now);
      entry.againstItself = compare(again, now);
    }
...
for (const [name, r] of Object.entries(report)) {
  ...
  if (old) {
    const { frames, max } = r.againstPrevious;
    assert(frames > 0.9 * r.frames && max.x === 0 && max.y === 0 && max.z === 0, `${name}: position differs from the previous build ${JSON.stringify(r.againstPrevious)}`);
  }
}
