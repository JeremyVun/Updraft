...
try {
  const result = await page.evaluate(async () => {
    ...
    const source = target => ({...baseState,music:target==='birches'?'birches':target==='sleeping'?'wood':target==='drowned'?'birches':target==='home'?'home':'sea',
      seaScore:target==='mirror'?'arrival':undefined,flockChatter:false});
    for(const target of Object.keys(ARRIVAL_MUSIC)) {
      const {ctx,sound}=offlineSound(29), notes=[], phases=[];
      ...
      const score=()=>sound.dreamScore??sound.linesScore??sound.boatsScore??sound.birchesScore??sound.sleepingScore;
      const update=tick=>{
        const now=tick/8, landed=now>=22;
        ...
        if(now===14){incoming=score();epoch=incoming?.current?.epoch??incoming?.epoch;}
        if(now===21){
      ...
      for(let tick=1;tick<29*8;tick++){await pause;update(tick);if(tick+1<29*8)pause=ctx.suspend((tick+1)/8);await ctx.resume();}
      const buffer=await rendering;
      let gapPeak=0, before=0, after=0;
      for(let ch=0;ch<2;ch++){
        const data=buffer.getChannelData(ch);
        for(let i=9.51*24000;i<9.89*24000;i++)gapPeak=Math.max(gapPeak,Math.abs(data[Math.floor(i)]));
        for(let i=6*24000;i<7*24000;i++)before+=data[i]**2;
        for(let i=18*24000;i<21*24000;i++)after+=data[i]**2;
      }
      check(gapPeak===0,`${target}: short background gap is digitally silent, including reverb`);
    ...
    transition.update(s,0);transition.update(s,2);
    check(Array.from({length:100},()=>transition.update(s,2).stage).every(stage=>stage==='gap'),'Suspended audio time preserves the gap');
    check(transition.update({...s,arrivalMusic:undefined,music:'lines',linesScore:'first'},2.2).stage==='gap','An early landing cannot cancel the pause');
    ...
    check(stalled.update(s,10).stage==='gap'&&stalled.update(s,10.39).stage==='gap'&&stalled.update(s,10.41).stage==='incoming',
      'A long stalled frame still gives retired sources the full short rest');
    const missedFade=new ArrivalTransition();missedFade.update(s,0);
    check(missedFade.update(s,5).stage==='gap'&&missedFade.update(s,5.39).stage==='gap',
      'A frame near the end of the gap cannot reopen still-retiring sources');
    const lateLanding=new ArrivalTransition();lateLanding.update(s,0);lateLanding.update(s,2);
    ...
    const update=tick=>{const now=tick/8;sound.update(.125,{...source('lines'),...(now>=22?ARRIVAL_MUSIC.lines:{}),
      arrivalMusic:now>=8&&now<22?'lines':undefined,gust:now>=11&&now<11.5?8:0});};
    update(0);let pause=ctx.suspend(.125);const rendering=ctx.startRendering();
    for(let tick=1;tick<29*8;tick++){await pause;update(tick);if(tick+1<29*8)pause=ctx.suspend((tick+1)/8);await ctx.resume();}
    ...
    check(power(10,10.5)>1e-8,'The environment remains audible during the background pause');
    check(power(11.1,11.5)>power(10,10.5)*2,'Wind and gesture feedback remain audible during the pause');
